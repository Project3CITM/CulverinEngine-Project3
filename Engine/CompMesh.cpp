		...
		//--------------this
			...
			if (resource_mesh->vertices.size() > 0)
			{
				...
				if (skeleton != nullptr)
				{
					...
					glVertexAttribPointer(4, 4, GL_INT, GL_FALSE, total_save_buffer * sizeof(GLfloat), (char *)NULL + (12 * sizeof(float)));
				}
...
void CompMesh::GenSkeleton()
{
	...
	//skeleton->skinning_mats = new GLfloat[skeleton->bones.size() * 4]; //every vertex will be influenced by 4 float4x4
