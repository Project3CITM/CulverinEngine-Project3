...
update_status Hardware::Update(float dt)
{
	if (active[0].active)
	{
		if (!ImGui::Begin("Hardware", &App->gui->win_manager[HARDWARE]->active[0].active, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize)) //TODO ELLIOT CLOSE Windows example
		{
			ImGui::End();
