...
vec3 blinnPhongDir(Light light, float Kd, float Ks, float shininess, vec3 N, float shadow)
{
    ...
    if (light.type != 0) {
        ...
        float attenuation =lightInt/pow(((d/light.radius)+1),2);//1/(light.properties[1] + light.properties[2]* d + light.properties[3] * d*d);
int power = 1;
attenuation = 1 - (pow(d,power)/pow(light.radius,power));

attenuation = lightInt * max(0, attenuation);
