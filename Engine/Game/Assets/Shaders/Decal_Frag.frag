...
vec3 blinnPhongDir(Light light, float Kd, vec3 N, float shadow)
{
    vec3 v = normalize(_cameraPosition - ourPos);
    ...
    if (light.type != 0) {
        ...
        float spec =  lightInt* pow(cosAlpha,50);
        ...
        vec3 s =  normalize(lightpos - ourPos);      // Light direction
        vec3 r = reflect(-s,N);

        float cosTheta = clamp( dot( s,N ), 0,1 ) ;
        float cosAlpha = clamp( dot( v,r ), 0,1 );

        float d = length((lightpos - ourPos));
        ...
        float spec = attenuation * pow(cosAlpha,50);
...
float CalcShadow(vec4 shadowPos, float usedBias)
{
    ...
    for(int i = 0; i < iterations; ++i)
    {
        ...
        float tmp = 0.1* shadowVal;// old value: 0.05 - iterations: 20
...
void main()
{
    vec4 color_texture_alpha =  texture(albedo, TexCoord);
    vec3 color_texture = color_texture_alpha.xyz;
 ...
 for (int i = 0; i <_numLights; ++i) {
       ...
       inten = blinnPhongDir(_lights[i], a_Kd,mat3(model) * ourNormal, shadow);
       inten_final += inten;
    ...
    vec3 col = max( color_texture * vec3(0,0.2,0.2), color_texture * (inten_final.x + inten_final.y)*final_color.rgb );
