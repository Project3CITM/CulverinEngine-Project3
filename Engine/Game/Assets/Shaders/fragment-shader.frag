...
uniform float diffuseCoefficient2;
...
vec3 ApplyLight(Light light,int type, vec3 surfaceColor, vec3 normal, vec3 surfacePos, vec3 surfaceToCamera) {
    ...
    if(type != 0) {
        ...
        attenuation = 1.0 / (1.0 + light.attenuation * pow(distanceToLight, intensity));
///////
...
//surfaceToLight = vec3(1);
    ...
    //diffuse
    float diffuseCoefficient = max(0, dot(normal, surfaceToLight));
    vec3 diffuse = surfaceColor.rgb *  diffuseCoefficient;// * light.l_color.rgb;//diffuseCoefficient * surfaceColor.rgb * light.l_color.rgb;
...
void main() {
    ...
    for(int i = 0; i < _numLights; ++i){
        linearColor += ApplyLight(_lights[i],_lights[i].type, surfaceColor.rgb, normal, surfacePos, surfaceToCamera);
    } //OurNormal = normal
