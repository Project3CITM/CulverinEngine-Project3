...
public class ShieldGuard_Listener : PerceptionListener
{
        ...
        switch (event_recieved.type)
        {
            ...
            case PERCEPTION_EVENT_TYPE.HEAR_WALKING_PLAYER:
                ...
                if (OnHearRange(event_recieved))
                {
                    ...
                    {
                        my_self.GetComponent<EnemyShield_BT>().heard_something = true;
                        ...
                        my_self.GetComponent<EnemyShield_BT>().InterruptAction();
            ...
            case PERCEPTION_EVENT_TYPE.PLAYER_SEEN:
                my_self.GetComponent<EnemyShield_BT>().InterruptAction();
                my_self.GetComponent<EnemyShield_BT>().player_detected = true;
                player_seen = true;
    ...
    public override void OnEventGone(PerceptionEvent event_recieved)
    {
        switch (event_recieved.type)
        {
            case PERCEPTION_EVENT_TYPE.HEAR_EXPLORER_EVENT:
            case PERCEPTION_EVENT_TYPE.HEAR_WALKING_PLAYER:
                my_self.GetComponent<EnemyShield_BT>().heard_something = false;
                my_self.GetComponent<Investigate_Action>().forgot_event = true;
                Debug.Log("I lost Somethin");
                break;

            case PERCEPTION_EVENT_TYPE.PLAYER_SEEN:
                my_self.GetComponent<EnemyShield_BT>().player_detected = false;
